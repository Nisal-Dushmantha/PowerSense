...
const sendSMS = async (toNumber, message) => {
  try {
    ...
    console.log(`[SMS] Message sent to ${toNumber} — SID: ${result.sid}`);
...
/**
 * Build and send the unpaid bill alert message via SMS
 * @param {object} user - User document (must include firstName, phoneNumber)
 * @param {object} bill - MonthlyBill document
 */
const sendUnpaidBillAlertSMS = async (user, bill) => {
  if (!user.phoneNumber) {
    console.warn(`[SMS] User ${user._id} has no phone number — skipping alert.`);
    return;
  }
...
module.exports = { sendSMS, sendUnpaidBillAlertSMS };
